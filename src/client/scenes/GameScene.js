...
this.musicaNivel.play({ loop: true, volume: 0.5 });
  ...
  scheduleNextPowerup() {
    ...
    if (this.powerupsUsedP1 >= 3 && this.powerupsUsedP2 >= 3) return; // No programar si ambos han usado 3 o más
  ...
  pickupPowerupByPlayer(playerId) {
    ...
    if (playerId === 1) {
      ...
      if (this.powerupStoredP1.length >= this.powerupMaxStored) return false;
      this.powerupStoredP1.push(this.currentPowerupType);
    } else {
      if (this.powerupsUsedP2 >= 3) return false;
      if (this.powerupStoredP2.length >= this.powerupMaxStored) return false;
  ...
  updatePowerupUI() {
    if (this.powerupTextP1) this.powerupTextP1.setText(`P1 Powerups: ${this.powerupStoredP1.length}/${this.powerupMaxStored}`);
    if (this.powerupTextP2) this.powerupTextP2.setText(`P2 Powerups: ${this.powerupStoredP2.length}/${this.powerupMaxStored}`);
  }
